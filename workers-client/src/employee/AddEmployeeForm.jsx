...
import Alert from '@mui/material/Alert';import { Button, Dialog, RadioGroup, TextField, FormControlLabel, Radio, InputLabel, MenuItem, Select } from "@mui/material";
...
const AddEmployeeForm = observer(({ handleClose, isOpen }) => {
    ...
    function onSubmit(data) {
        ...
        if (isReadyToSend) {
            ...
            data.roles.map(role => role.isAdministrative == Boolean(data.isAdministrative))
            if (currentEmployee) {
    ...
    return (
        <Dialog onClose={handleClose} open={isOpen} sx={{ '& .MuiDialog-paper': { width: '80%', overflow: 'auto' } }}
        ...
        >
            ...
            >
                {!isReadyToSend && <Alert severity="error">You choose the same role twice!</Alert>}
                <FormControl>
                ...
                {fields.map((item, index) => (
                    <CardContent key={item.id} sx={{
                        ...
                        <FormControl>
                            ...
                            >
                                {roleStore.roles.map(role => <MenuItem key={role.id} value={role.name}>{role.name}</MenuItem>)}
                        ...
                        <Button type="button" startIcon={<DeleteIcon />} onClick={(event) => {
                            remove(index)
